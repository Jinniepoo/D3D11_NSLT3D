...
HRESULT CGraphic_Device::Ignore_ShaderWarning()
{
	vector<D3D11_MESSAGE_ID> IdVec;
	IdVec.push_back(D3D11_MESSAGE_ID_SETPRIVATEDATA_CHANGINGPARAMS);

	ID3D11InfoQueue* InfoQueue = nullptr;
	if (FAILED(m_pDevice->QueryInterface(__uuidof(ID3D11InfoQueue), reinterpret_cast<void**>(&InfoQueue))))
		return E_FAIL;

	D3D11_INFO_QUEUE_FILTER Filter = {};
	Filter.DenyList.NumIDs = static_cast<_uint>(IdVec.size());
	Filter.DenyList.pIDList = IdVec.data();
