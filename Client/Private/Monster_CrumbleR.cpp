#include "stdafx.h"
#include "..\Public\Monster_CrumbleR.h"
...
CMonster_CrumbleR::CMonster_CrumbleR(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
...
CMonster_CrumbleR::CMonster_CrumbleR(const CMonster_CrumbleR& rhs)
	: CGameObject(rhs)
...
HRESULT CMonster_CrumbleR::Initialize_Prototype()
{
...
HRESULT CMonster_CrumbleR::Initialize(void* pArg)
{
	if (nullptr != pArg)
...
void CMonster_CrumbleR::PriorityTick(_float fTimeDelta)
{
}

void CMonster_CrumbleR::Tick(_float fTimeDelta)
{
...
void CMonster_CrumbleR::LateTick(_float fTimeDelta)
{
...
HRESULT CMonster_CrumbleR::Render()
{
...
HRESULT CMonster_CrumbleR::Ready_Components()
{
	if (LEVEL_GAMEPLAY == m_eLevel)
	{
		...
		/* For.Com_Model */
		if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_Monster_CrumbleR"),
	...
	if (LEVEL_PUZZLE == m_eLevel)
	{
		...
		/* For.Com_Model */
		if (FAILED(__super::Add_Component(LEVEL_PUZZLE, TEXT("Prototype_Component_Model_Monster_CrumbleR"),
			TEXT("Com_Model"), reinterpret_cast<CComponent**>(&m_pModelCom))))
			return E_FAIL;
	...
	if (LEVEL_BOSS == m_eLevel)
	{
		...
		/* For.Com_Model */
		if (FAILED(__super::Add_Component(LEVEL_BOSS, TEXT("Prototype_Component_Model_Monster_CrumbleR"),
			TEXT("Com_Model"), reinterpret_cast<CComponent**>(&m_pModelCom))))
			return E_FAIL;
	}
...
HRESULT CMonster_CrumbleR::Bind_ShaderResources()
{
...
_bool CMonster_CrumbleR::Near_Player()
{
...
void CMonster_CrumbleR::Chase_Player(_float fTimeDelta)
{
	_float3 vPlayerPos;
	_float3 vMonsterPos;
...
void CMonster_CrumbleR::Move_Randomly(_float fTimeDelta)
{
...
HRESULT CMonster_CrumbleR::Drop_Coins(_vector vMonsterPos)
{
	_float4x4 matPos;
...
HRESULT CMonster_CrumbleR::Startle_Effect(_vector vMonsterPos)
{
...
HRESULT CMonster_CrumbleR::Dizzy_Effect(_vector vMonsterPos)
{
...
CMonster_CrumbleR* CMonster_CrumbleR::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CMonster_CrumbleR* pInstance = new CMonster_CrumbleR(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CMonster_CrumbleR"));
...
CGameObject* CMonster_CrumbleR::Clone(void* pArg)
{
	CMonster_CrumbleR* pInstance = new CMonster_CrumbleR(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CMonster_CrumbleR"));
		Safe_Release(pInstance);
	}
