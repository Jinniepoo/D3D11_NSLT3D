...
CCollectibles_Boss::CCollectibles_Boss(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject{ pDevice, pContext }
{
}

CCollectibles_Boss::CCollectibles_Boss(const CCollectibles_Boss& rhs)
	: CGameObject(rhs)
...
HRESULT CCollectibles_Boss::Initialize_Prototype(const _tchar* strComponentTag)
{
...
HRESULT CCollectibles_Boss::Initialize(void* pArg)
{
	if (nullptr != pArg)
...
void CCollectibles_Boss::PriorityTick(_float fTimeDelta)
{
...
void CCollectibles_Boss::Tick(_float fTimeDelta)
{
	if (m_CltbDesc.m_CltbState != CLTB_DROP && m_CltbDesc.m_CltbType != TYPE_BURROWDIRT)
	{
		...
		CCollider* pTargetCollider = dynamic_cast<CCollider*>(m_pGameInstance->Find_Component(LEVEL_PUZZLE, L"Layer_Player", L"Com_Collider", 0));
		CCollider* pTailCollider = dynamic_cast<CCollider*>(m_pGameInstance->Find_Component(LEVEL_PUZZLE, L"Layer_Player", L"Com_TailCollider", 0));
...
void CCollectibles_Boss::LateTick(_float fTimeDelta)
{
...
HRESULT CCollectibles_Boss::Render()
{
...
HRESULT CCollectibles_Boss::Ready_Components()
{
	/* For.Com_Shader */
...
HRESULT CCollectibles_Boss::Bind_ShaderResources()
{
...
void CCollectibles_Boss::Save_ModelData(HANDLE hFile, _ulong& dwByte)
{
	const _float4x4 WorldMatrix = *(m_pTransformCom->Get_WorldFloat4x4());
...
void CCollectibles_Boss::LootEffect()
{
...
void CCollectibles_Boss::DropEffect()
{

}

void CCollectibles_Boss::UIBoardUpdate()
{
	if (m_CltbDesc.m_CltbType == TYPE_COIN)
		m_pUICoin->Set_CoinScore();
...
CCollectibles_Boss* CCollectibles_Boss::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const _tchar* strComponentTag)
{
	CCollectibles_Boss* pInstance = new CCollectibles_Boss(pDevice, pContext);
...
CGameObject* CCollectibles_Boss::Clone(void* pArg)
{
	CCollectibles_Boss* pInstance = new CCollectibles_Boss(*this);
