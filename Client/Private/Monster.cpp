...
HRESULT CMonster::Ready_Components()
{
	if (LEVEL_GAMEPLAY == m_eLevel)
	{
		...
		/* For.Com_Model */
		if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_Monster_Wriggle"),
	...
	if (LEVEL_PUZZLE == m_eLevel)
	{
		...
		/* For.Com_Model */
		if (FAILED(__super::Add_Component(LEVEL_PUZZLE, TEXT("Prototype_Component_Model_Monster_Wriggle"),
			TEXT("Com_Model"), reinterpret_cast<CComponent**>(&m_pModelCom))))
			return E_FAIL;
	...
	if (LEVEL_BOSS == m_eLevel)
	{
		...
		/* For.Com_Model */
		if (FAILED(__super::Add_Component(LEVEL_BOSS, TEXT("Prototype_Component_Model_Monster_Wriggle"),
			TEXT("Com_Model"), reinterpret_cast<CComponent**>(&m_pModelCom))))
			return E_FAIL;
	}
...
void CMonster::Move_Randomly(_float fTimeDelta)
{
...
HRESULT CMonster::Drop_Coins(_vector vMonsterPos)
{
	_float4x4 matPos;
...
HRESULT CMonster::Startle_Effect(_vector vMonsterPos)
{
	CEffect_Startle::EFFECT_DESC StartleDesc;

	vMonsterPos = XMVectorAdd(vMonsterPos, XMVectorSet(0.f, 4.f, 0.f, 0.f));
...
HRESULT CMonster::Dizzy_Effect(_vector vMonsterPos)
{
