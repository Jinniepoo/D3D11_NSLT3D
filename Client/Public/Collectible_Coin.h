...
BEGIN(Client)
class CCollectible_Coin : public CGameObject
{
public:
...
private:
	CCollectible_Coin(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CCollectible_Coin(const CCollectible_Coin& rhs);
	virtual ~CCollectible_Coin() = default;
...
//	void		Set_CoinScore(_int iScore) { m_iCoinScore = iScore; }
...
public:
	static CCollectible_Coin* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const _tchar* strComponentTag);
